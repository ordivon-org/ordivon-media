...
A3-1 through A3-3 established that the cross-medium model can change real production decisions and can be falsified by rendered artifacts. Cultural Observatory R0-R3 proved the first live outer-loop slice. R4 pressure-tested richer perception and separated perception bandwidth from explanatory power. R5 then made meaning itself experimental: Provider observations were required to cite exact evidence, controlled omission exposed plausible but unsupported event inference, fine discourse/speech labels proved substantially less stable than their grounding, and a separate Agent-observer knowledge-transfer bridge reached 97.9% grounded substantive acquisition across an eight-replicate follow-up while the no-artifact control remained 100% abstaining with zero unsupported assertions. The next pressure is **controlled meaning intervention plus real consequence**: change one evidence-grounded expressive relation in owned artifacts, expose variants under an explicit encounter, and measure the consequence type that the claim actually concerns rather than expanding the ontology.
