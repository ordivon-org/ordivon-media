...
A3-1 through A3-3 established that the cross-medium model can change real production decisions and can be falsified by rendered artifacts. Cultural Observatory R0-R3 proved the first live outer-loop slice. R4 separated perception bandwidth from explanatory power. R5 established evidence-addressed meaning and typed Agent-observer consequence. R6 then added the missing research-validity and encounter layer: high-budget zero-effect search manufactured spectacular false winners, full-search replay exposed selection inflation, one correctly calibrated adaptive test still false-positived, and a pristine holdout rejected it. Web subsequently proved explicit assignment/propensity/realized-exposure receipts over real Chromium encounters. A controlled same-fact reveal-order intervention produced a +18.75pp visible Agent-observer adaptation difference for explicit-chain versus fragmented presentation; one physically sealed content holdout preserved the direction at +4.17pp while a post-freeze bootstrap showed substantial uncertainty and effect shrinkage. [`experiments/r6-creative-alpha.md`](./experiments/r6-creative-alpha.md) records the full boundary.

M7 then expanded the **media scope itself** rather than optimizing one medium. Deletion, wrong-profile, sham, encounter-freezing and ROI pressure produced an intentionally asymmetric result: Writing and Motion/Video remain active controls; Audio graduated to an active profile; Still Visual and generic Interactive remain compact provisional candidates; Spatial/3D and Live/Realtime remain foundation candidates; Haptic/Physical remains a challenger without a profile. The old bundled `medium-baseline` placeholder was deleted because it hid those different evidence states. [`experiments/m7-foundational-media.md`](./experiments/m7-foundational-media.md) records the result and [`media-world-model.json`](./media-world-model.json) is the compact Agent-readable topology.

The next pressure is ordinary consumption rather than adding categories: use the retained model during real Studio/Web work, let non-Web interactive work decide whether generic Interactive deserves promotion, and require real spatial/live/tactile encounters before those candidates gain profile or Production-schema authority.
