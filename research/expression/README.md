...
A3-1 through A3-3 established that the cross-medium model can change real production decisions and can be falsified by rendered artifacts. Cultural Observatory R0-R3 proved the first live outer-loop slice. R4 then pressure-tested richer perception: full-article mechanics failed to produce a pooled attention-selection gain and showed section-level reversals, while video temporal and audio spectral/energy profiles cleanly detected controlled reorder interventions hidden from shallow metadata. Audiovisual coupling earned only bounded diagnostic status. The next pressure is **meaning and consequence**, not more mechanical feature accumulation: semantic/rhetorical and event-level perception through admitted Provider authority, followed by controlled interventions and owned-surface comprehension, retention, behavior, or memory evidence.
