...
This closes the compatibility question for the current workstation profile. Native OTIO conform is now the accepted assembly mechanism. The explicit source-range Append case remains only inside the version-specific compatibility probe because it is useful for detecting Resolve semantic drift. The former manual `assemble-review` / `prepare-assembly` production path was retired after the native conform acceptance: shared Production/Asset/OTIO validation and segment compilation remain, but there is no second executable assembly effect path.
